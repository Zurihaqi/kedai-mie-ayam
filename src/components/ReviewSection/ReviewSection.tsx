...
import avatar from "../../../public/default_pfp.png";
...
const ReviewSummary: React.FC<ReviewSummaryProps> = ({ title, ratings }) => {
...
const Review: React.FC<Review> = ({ user, rating, comment, date }) => {
  const renderStars = () => {
    const stars = [];
    const totalStars = 5;
      ...
      <div className="flex flex-wrap justify-between">
        <div className="flex flex-row">
          <Image
            ...
            width={30}
            height={30}
            className="rounded-full mx-2 w-full"
        ...
        <div className="">
          <p className="text-gray-500 flex-1">{date}</p>
        </div>
      </div>
  ...
  return (
    <div className="flex lg:flex-row flex-col lg:gap-10 gap-2 items-center justify-center">
      <div className="lg:self-start lg:mt-20">
        <ReviewSummary title="Ringkasan Ulasan" ratings={ratings} />
      </div>
